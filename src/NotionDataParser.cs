...
namespace Flow.Launcher.Plugin.Notion
{
...

    {
        ...
        internal NotionDataParser(PluginInitContext context, Settings settings)
        {
            ...
            _iconPath = Path.Combine(_context.CurrentPluginMetadata.PluginDirectory,"icons","icons");
        }
        ...
        static string ConvertVariables(string input)
        {
            ...
            foreach (Match bracketMatch in bracketMatches)
            {
                ...
                string[] IsContainDate = next_occurrence_of_day(textInsideBrackets);
                if (!string.IsNullOrEmpty(IsContainDate[0]))
                {
                    var replacementText = Convert.ToDateTime(next_occurrence_of_day(textInsideBrackets)[0]).ToString("yyyy-MM-ddTHH:mm:ssZ");
        ...
        static string[] next_occurrence_of_day(string input = "")
        {
